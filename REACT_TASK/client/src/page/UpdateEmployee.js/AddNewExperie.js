...
const UpdateExperienceDetails = () => {
    ...
    defaultValues : {
        input : selectedEmployee?.ExperienceDetails // [{company : "nikul" , designation : "nik" , department : "sdfsdfsdf" , ctc : 100 } ]
    },
    ...
    const onHandelSubmit = (data) => {
        DetailOfAllEmployee[selectedEmployee.id - 1 ].ExperienceDetails = data.input
        navigate("/update-EducationalDetails")
    }
    useEffect(() => {
        if(!selectedEmployee) {
            return
        ...
        <Link to="/update-CurrentStatus" ><button type="button">Back</button></Link>
          </form>
        } {!currentstatus &&
            <Link to="/update-CurrentStatus"> 
